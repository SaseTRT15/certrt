...
certrt_print_text($pdf, $x, $y + 55, 'C', $fontserif, '', 20, get_string('certify', 'certrt'));
certrt_print_text($pdf, $x, $y + 105, 'C', $fontserif, '', 30, fullname($USER));
certrt_print_text($pdf, $x, $y + 155, 'C', $fontserif, '', 20, get_string('statement', 'certrt'));
certrt_print_text($pdf, $x, $y + 205, 'C', $fontserif, '', 20, format_string($course->fullname));
certrt_print_text($pdf, $x, $y + 255, 'C', $fontserif, '', 14, certrt_get_date($certrt, $certrecord, $course));
certrt_print_text($pdf, $x, $y + 283, 'C', $fontserif, '', 10, certrt_get_grade($certrt, $course));
certrt_print_text($pdf, $x, $y + 311, 'C', $fontserif, '', 10, certrt_get_outcome($certrt, $course));
if ($certrt->printhours) {
    certrt_print_text($pdf, $x, $y + 339, 'C', $fontserif, '', 10, get_string('credithours', 'certrt') . ': ' . $certrt->printhours);
}
...
if ($certrt->printteacher) {
    ...
    if ($teachers = get_users_by_capability($context, 'mod/certrt:printteacher', '', $sort = 'u.lastname ASC', '', '', '', '', false)) {
        foreach ($teachers as $teacher) {
            $i++;
            certrt_print_text($pdf, $sigx, $sigy + ($i * 12), 'L', $fontserif, '', 12, fullname($teacher));
        }
    }
}
