...
/** VMF **/
/** The column image folder */
define('CERT_IMAGE_COLUMN', 'columns');
/* END VMF */
...
function certrt_get_images($type) {
    ...
    switch($type) {
        case CERT_IMAGE_BORDER :
            ...
            break;
        /* VMF */
        case CERT_IMAGE_COLUMN :
            $path = "$CFG->dirroot/mod/certrt/pix/columns";
            $uploadpath = "$CFG->dataroot/mod/certrt/pix/columns";
            break;
        /* END VMF */
...
function certrt_get_date($certrt, $certrecord, $course, $userid = null) {
    ...
    if ($certrt->printdate > 0) {
        ...
        } else if ($certrt->datefmt == 2 || $certrt->datefmt == 3) {
            $suffix = certrt_get_ordinal_number_suffix(userdate($date, '%d'));
            $certrtdate = userdate($date, get_string('certdateformat', 'certrt',$suffix));
...
function certrt_print_image($pdf, $certrt, $type, $x, $y, $w, $h) {
    ...
    switch($type) {
        ...
        case CERT_IMAGE_COLUMN :
            $attr = 'columnimage';
            $path = "$CFG->dirroot/mod/certrt/pix/$type/$certrt->columnimage";
            $uploadpath = "$CFG->dataroot/mod/certrt/pix/$type/$certrt->columnimage";
            break;
        /* END VMF */
        case CERT_IMAGE_SEAL :
            $attr = 'printseal';
...
function certrt_get_certrt_filename($certrt, $cm, $course) {
   ...
   /* VMF */
    ...
    function defaultVerso($course_id) {
        ...
        $texto .= get_string('versedefaulthtml', 'certrt');

        $result = $DB->get_records_select('course_sections', "course = $course_id");

        foreach ($result as $record) {
            if ($record->name){
                $texto .= "<p>$record->name</p>"; 
            }
        }        
        return $texto;
    }
