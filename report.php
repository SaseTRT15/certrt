...
// the user has not specified they wish to view all certrts.
if (CERT_PER_PAGE !== 0) {
...
$url = new moodle_url('/mod/certrt/report.php', array('id'=>$id, 'page' => $page, 'perpage' => $perpage));
if ($download) {
...
if (!$certrt = $DB->get_record('certrt', array('id'=> $cm->instance))) {
    print_error('certrt ID was incorrect');
}
...
require_capability('mod/certrt:manage', $context);

// Declare some variables
$strcertrts = get_string('modulenameplural', 'certrt');
$strcertrt  = get_string('modulename', 'certrt');
$strto = get_string('awardedto', 'certrt');
$strdate = get_string('receiveddate', 'certrt');
$strgrade = get_string('grade','certrt');
$strcode = get_string('code', 'certrt');
$strreport= get_string('report', 'certrt');

if (!$download) {
    $PAGE->navbar->add($strreport);
    $PAGE->set_title(format_string($certrt->name).": $strreport");
...
if (!$users = certrt_get_issues($certrt->id, $DB->sql_fullname(), $groupmode, $cm, $page, $perpage)) {
    echo $OUTPUT->header();
    groups_print_activity_menu($cm, $CFG->wwwroot . '/mod/certrt/report.php?id='.$id);
    echo $OUTPUT->notification(get_string('nocertrtsissued', 'certrt'));
...
if ($download == "ods") {
    ...
    // Calculate file name
    $filename = certrt_get_certrt_filename($certrt, $cm, $course) . '.ods';
    // Creating a workbook
    ...
    if ($users) {
        foreach ($users as $user) {
            ...
            $myxls->write_string($row, $nextposition + 2, certrt_get_grade($certrt, $course, $user->id));
...
if ($download == "xls") {
    ...
    // Calculate file name
    $filename = certrt_get_certrt_filename($certrt, $cm, $course) . '.xls';
    // Creating a workbook
...
if ($download == "txt") {
    $filename = certrt_get_certrt_filename($certrt, $cm, $course) . '.txt';
    ...
    if ($users) foreach ($users as $user) {
        ...
        echo certrt_get_grade($certrt, $course, $user->id) . "\t";
        echo $user->code . "\n";
        $row++;
    }
    exit;
...
$usercount = count(certrt_get_issues($certrt->id, $DB->sql_fullname(), $groupmode, $cm));
...
foreach ($users as $user) {
    ...
    $date = userdate($user->timecreated) . certrt_print_user_files($certrt, $user->id, $context->id);
    $code = $user->code;
    $data = array();
    $data[] = $name;
    foreach ($extrafields as $field) {
    ...
    $data = array_merge($data, array($date, certrt_get_grade($certrt, $course, $user->id), $code));
    $table->data[] = $data;
}
...
echo $OUTPUT->header();
groups_print_activity_menu($cm, $CFG->wwwroot . '/mod/certrt/report.php?id='.$id);
echo $OUTPUT->heading(get_string('modulenameplural', 'certrt'));
